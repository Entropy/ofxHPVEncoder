...
namespace HPV {
	...
	*/
	...
	public:
		...
		int write(const char * buf, uint64_t frame_size)
		{
			if (bInit)
			{
				bool bOK = true;
				...
				if (!bOK)
					HPV_ERROR("Error writing frame at current pos");
    ...
    public:
		...
		int init(const HPVCreatorParamsInline& paramsInLine, ThreadSafe_Queue<HPVCompressionProgress> * progress_sink);
		int process_sequence(std::size_t amount_of_concurrency);
        void process_item(uint8_t thread_idx);
		bool process_frame(uint8_t * frame, const HPVCompressionWorkItem & item);
    ...
    private:
		...
		HPVFileStreamWriter fs;

		std::vector<uint32_t> frame_size_table;
		std::mutex frame_size_mutex;
