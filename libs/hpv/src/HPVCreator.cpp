...
namespace HPV {
    ...
    int HPVCreator::init(const HPVCreatorParams& _params, ThreadSafe_Queue<HPVCompressionProgress> * _progress_sink)
	{
		...
		try{
			frame_size_table.resize(compression_queue.size());
		}catch(...){
		...
		fs.write_to_stream( (const char *)frame_size_table.data(), bytes_in_header, bytes_in_framesize_table);
	...
	int HPVCreator::init(const HPVCreatorParamsInline& _params, ThreadSafe_Queue<HPVCompressionProgress> * _progress_sink){
		if (file_counter > 0)
		{
			reset();
		}
		...
		if (!fs.init(outpath))
		{
			HPV_ERROR("Error while opening tmp path %s", outpath.c_str());
		}
    ...
    void HPVCreator::coordinate(uint8_t num_threads)
	{
        ...
        while (should_coordinate.load())
        {
			...
			if (!in_line && items_done_counter == length && compression_queue.empty())
            {
                should_coordinate.store(false, std::memory_order_relaxed);
            }
        }
		...
		if(in_line){
			// write our frame sizes table at the end of the file
			bytes_in_framesize_table = frame_size_table.size() * sizeof(uint32_t);
			fs.write( (const char *)frame_size_table.data(), bytes_in_framesize_table);
			length = items_done_counter;
		}else{
		...
		fs.write_to_stream((const char *)&compressed_total_size, 28, 4);
	...
	bool HPVCreator::process_frame(uint8_t * pixels, const HPVCompressionWorkItem & item){
		std::size_t compressed_size = 0;


		if (!pixels)
		{
			error.done_item_name = "Failed compressing null pixels ";
			progress_sink->push(error);
			return false;
		}
		...
		if (HPVCompressionType::HPV_TYPE_DXT1_NO_ALPHA == type)
		{
			rygCompress(dxt, pixels, ref_width, ref_height, false);
		}
		else if (HPVCompressionType::HPV_TYPE_DXT5_ALPHA == type)
		{
			rygCompress(dxt, pixels, ref_width, ref_height, true);
		}
		else if (HPVCompressionType::HPV_TYPE_SCALED_DXT5_CoCg_Y == type)
		{
			// YCoCg conversion if DXT5
			ConvertRGBToCoCg_Y(pixels, ref_width, ref_height);

			// DXT compress
			CompressYCoCgDXT5(pixels, dxt, ref_width, ref_height, ref_width * 4);
		}
		...
		// write compressed size to frame size index table
		{
			std::unique_lock<std::mutex> lck(frame_size_mutex);
			if(frame_size_table.size() < item.offset + 1){
				frame_size_table.resize(item.offset + 1);
			}
		}
		frame_size_table[item.offset] = static_cast<uint32_t>(compressed_size);
	...
	{
		if (!in_line && !compression_queue.size())
            return HPV_RET_ERROR;
    ...
    void HPVCreator::stop()
    {
		...
		if(in_line){
			HPVCompressedItem compressed_item;
			compressed_item.frame_size          = 0;
			filestream_queue.push(compressed_item, items_done_counter);
		}
